...
function BookController(){
	...
	this.uppaperBook = function (req, res){
		...
		// 封面存文件
		var name = ((new Date().getTime())*Math.random()*100).toFixed(0)+'.'+req.files.cover.mimetype.split('/')[1];
		var path = path.join(__dirname,'../img/'+name);

			fs.writeFile(path, fs.readFileSync(req.files.cover.path), 'utf8', function(err){
				if (err) throw err;
				console.log('ok')
				return
			});
